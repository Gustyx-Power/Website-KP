...
<svelte:head>
	<title>Manajemen Penjualan - AdminPanel</title>
</svelte:head>

<div class="mb-6">
	<h2 class="text-2xl font-bold text-slate-800">Manajemen Penjualan</h2>
	<p class="text-slate-500 mt-1">Catat transaksi penjualan dan kurangi stok otomatis.</p>
</div>

<div class="bg-white rounded-xl shadow-sm border border-slate-100 p-6 mb-8">
	<h3 class="text-lg font-semibold text-slate-800 mb-4">Input Penjualan Baru</h3>
	<form method="POST" action="?/create" class="space-y-4">
		<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
			...
			<div>
				<label for="qty_terjual" class="block text-sm font-medium text-slate-700 mb-1"
					>Qty Terjual *</label
				>
				<input
					type="number"
			...
			<div>
				<label for="total_uang" class="block text-sm font-medium text-slate-700 mb-1"
					>Total Uang (Rp) *</label
				>
				<input
					type="number"
		...
		<table class="w-full text-left border-collapse">
			<thead>
				<tr class="bg-slate-50 border-b border-slate-100">
					...
					<th class="py-4 px-6 font-medium text-slate-600 text-right">Qty Terjual</th>
					<th class="py-4 px-6 font-medium text-slate-600 text-right">Total Uang</th>
				</tr>
			</thead>
			<tbody class="divide-y divide-slate-100">
				{#each data.penjualan as item}
					<tr class="hover:bg-slate-50 transition-colors">
						...
						<td class="py-3 px-6 text-right font-medium text-slate-800">{item.qty_terjual}</td>
						<td class="py-3 px-6 text-right text-slate-600"
							>Rp {item.total_uang.toLocaleString('id-ID')}</td
						>
					</tr>
				{:else}
					<tr>
						<td colspan="5" class="py-8 text-center text-slate-500">Belum ada data penjualan.</td>
					</tr>
				{/each}
			</tbody>
		</table>
