<script lang="ts">
	...
	async function exportPDF() {
		...
		// Handle Laporan Distribusi
		if (selectedLaporan === 'distribusi') {
			isExporting = true;
			try {
				const formData = new FormData();
				formData.append('periode', filterPeriode);
				...
				const response = await fetch('?/getDistribusiData', {
					method: 'POST',
					body: formData
				});
				...
				if (result.type === 'success') {
					...
					const { exportDistribusiPDF } = await import('$lib/exportUtils');
					await exportDistribusiPDF(reportData);
				} else {
					throw new Error(result.errors?.[0]?.message || 'Gagal mengambil data laporan');
				}
			} catch (error) {
